...
```{r c5-tmap-countries2, fig.show='hold', out.width="60%"}
...
```{r c5-tmap-countries4,fig.show="hold", out.width="60%"}
...
```{r c5-tmap-countries6, fig.show='hold', out.width="60%", fig.cap="Basic map without labels (left) and map with added coutries names (right)"}
...
```{r c5-tmap-countries-breaks,warnings = FALSE, message = FALSE,out.width="60%"}
#default settings
tm_shape(africountries) +
...
So far we visualised whole datasets but now let's assume that we are only interested in the subset of our data, for instance, a specific country or the airports of a specific size only. We can achieve it using `dplyr` package. In the example below, our focus is on the large airport, consequently we `filter` them based on their size. This process of subsetting can be decomposed into smaller steps. Firstly, we create a new dataset called *large_airports* which is based on the subset of data from the original dataset called *afriairports*. The data is subset using so-called pipe operator ` %>% ` that essentially links two arguments in the sequence. Here, ` %>% ` links our original dataset with the subsequent function `filter()`. The `filter()` function allows us to subset variables or dataset that meet our condition specified inside it. Specifically, we selected all the airports from the whole dataset which are large, based on the  *type* variable which contains *large_airport* observations that correspond to our airports of interest (large ones).<br/>

Next, we simply visualise two datasets in a single map. In the first step we we use *africountries* inside the `tm_shape()` function to create a base map of Africa. We specify that borders should be displayed with `tm_borders()`. In the second step we add new *large_airports* dataset with `tm_shape(large_airports)` and we specify that airport (from the *large_airports* dataset) should be visualised as red dots `tm_dots(col = "red")`.
...
Another useful, subsetting example may be to visualise only information for one, specific country. This can be done using similar approach as presented above. In our example, we will focus on presenting airports of Morocco on the map. Firstly, we take the dataset that contains all the airports (51 observations in total) and create a new dataset called *morocco* which contains only the airports of Morocco (31 observations in total). In the second step we create a map of the whole Africa coloured in beige and then overlay the moroccan airports as blue dots.
```{r c5-tmap-subset-2}
tmap_mode("plot")

morocco = afriairports %>% filter(country_name == "Morocco")
...
It is also possible to visualise the different sizes of airport in the subset data. It is done exactly the same way as in the case of the whole dataset, where the colour of the dots are set to column name that contains the size of the airport. For clarity, the code within `tm_dots()` function adjusts the colour and size of the airport dots and adds the legend.
...
However, it can also be observed that the view of the map is quite zoomed out and some of the airports overlap. A possible solution to this problem would be to subset both, airports of Morocco as well as the country borders so that only map of Morocco is visible which allows for taking a closer look at location of the airports. The rest of the code is identical to that above. The additional component of `tm_layout()` is used to control the position of the legend of the map. It is needed here as it was overlapping with the map in the default position (bottom, left corner). Alternatively, it is also possible to use an interactive map to freely zoom in and out. This will be presented in the following section.


```{r}

morocco_map=africountries %>%
  filter(`name_long` == "Morocco")

tm_shape(morocco_map)+
  tm_polygons(col = "lightyellow")+
  tm_shape(morocco) +
  tm_dots(col = "type", shape = 21, size = 0.2, title = "Airport size",
          labels = c("Large", "Medium", "Small"),
...
Finally, it is also possible to zoom the map in by cropping it along the coordinates. This can be done using `st_crop()` function where we need to specify boundary longitude and latitude in degrees along which the map will be cut. The min and max x correspond to longitude values and max and min y correspond to latitude. We can obtain the boundary coordinates of our map by running `st_box(function)`. We then specify aribtrary values along which we want to split the map in the `st_crop()` function as presented below where we focus only on the western Africa.


```{r, warning=FALSE, message=FALSE}
st_bbox(africontinent)

west <- st_crop(africountries, xmin = -17, xmax = 16,
                          ymin = 2, ymax = 21)
tm_shape(west)+
  tm_polygons()
...
```{r, warning=FALSE, message=FALSE}
west_airports <- st_crop(afriairports, xmin = -17, xmax = 16,
                ymin = 2, ymax = 21)
...
Finally, we arrive to visualisation of subset of data in an interactive mode. This only requires of change of view mode asimilar to examples above.
