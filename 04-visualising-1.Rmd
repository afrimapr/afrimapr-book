...
If we  the map of African population with default breaks, there is not enough differentiation for the map to be readable, as seen below.  
```{r }
data(afripop2020)
...
Therefore, manual breaks are a way to address this problem. For this, we set up breaks first, outside of the map-making code and then we use them to set up manual brakes for the map. This allows us to achieve better readable map and clarity. 

```{r }
breaks=c(0,1,10,100,1000, 25000)

tm_shape(afripop2020) +
  tm_raster(breaks=breaks)
```

Furthermore, the differences between levels of population density represented with raster data can be further emphasised by modifying a colour palette. In the example below, we specified it using `palette` argument within `tm_raster` function.

```{r }
  tm_shape(afripop2020) +   
    tm_raster(breaks=breaks, palette = rev(viridisLite::rocket(5))) 
```
...
DiagrammeR::grViz("digraph {
  ...
  node1 [ label = 'Step 1: Attribute-based subsetting of country']
  node2 [label = 'Step 2: Spatial subsetting of data']
