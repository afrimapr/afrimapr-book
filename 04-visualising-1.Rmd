...
If we use the default breaks in the map of African population, it is clear that there is not enough differentiation for the map to be readable, as seen below.  
```{r, out.width="60%"}
data(afripop2020)
...
Therefore, customised breaks are a way to address this problem. For this, we set up breaks first, outside of the map-making code and then we use them within the map-making code. This allows us to achieve better readable map and clarity. 

```{r, out.width="60%"}
raster_breaks=c(0,1,10,100,1000, 25000)

tm_shape(afripop2020) +
  tm_raster(breaks = raster_breaks)
```
