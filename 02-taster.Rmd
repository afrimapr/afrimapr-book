...
This chapter shows the steps through an example process of map-making. It demonstrates the focus and potential of this book, so that the readers can quickly see what this book aims to achieve and how this is done. 
...
## A simple map
```{r c2-simple-map, out.width="50%"}
tm_shape(africountries) +
    tm_polygons()
```
## Map components
```{r c2-simple-map, out.width="50%"}
tm_shape(africountries) + #dataset used to create a map
    tm_polygons(col = "area_sqkm", title = "Country size (sq km)") #function to draw polygons of the map, 'col' fills polygons based on the variable in the dataset, 'title' adjust the legend title
...
tm_shape(afripop2020) +
    tm_raster(breaks=breaks, title = "") +
tm_shape(afrihighway) +
    tm_lines(col = "red") + 
tm_shape(africapitals) +
    tm_symbols(col = "black", alpha=0.5, scale = 0.6 )+
