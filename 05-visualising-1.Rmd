...
We can further build on the previous map by adding labels for country names with `tm_text` function. In a similar manner a variety of different layers can be added. 
...
### Colour pallet
An important aspect of the maps, as in every visual representation, are the colours. In `tmap` we can change the default colours using `palette` argument. In the example below, we used palette called **Set1** and **plasma** and there are many other option to choose from. To view them, we run `tmaptools::palette_explorer()` which will open a new window with a wide range of palettes. Sliders on the left hand side allow selecting a number of colours. 
...
## Further resources
- [r documentation](https://www.rdocumentation.org/packages/tmap/versions/3.3-1)
- [get started with tmap](https://cran.r-project.org/web/packages/tmap/vignettes/tmap-getstarted.html#facets)
