...
<div class = "purple">
**Exercise 5.3.4**: Firstly, plot a map with continent contour in light green colour. Then add a layer of african highways. At the end include a title on the bottom of the map. Tip: For plotting highways use *afrihighway* data. 
</div>  
...
```
Using the map layer icon we can enable and disable the visibility of different map layers. Furthermore, `tm_tiles` function allows us to add a layer of overlay tiles. It is only available in "view" mode.
...
tm_shape(africa) + 
  tm_borders()+
  tm_bubbles(size = "pop_est", col = "pop_est") +
  tm_tiles("Stamen.TonerLabels") # adds overlay tiles with names
```
...
<div class = "purple">
**Exercise 5.4.1**: Plot a map that has polygons filled with gross domestic product, black borders, legend titled "GDP" and customised breaks at 0, 20000, 30000, 40000, 50000, 100000, 200000, 300000, 500000.
</div>  
<br/>
