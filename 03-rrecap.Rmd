...
### Data frame
Data frame is a two dimensional array, similar to a matrix. However, it can contain different data types. Beyond, columns as well as rows within the data frame are named. You can create a simple data frame with two columns and three rows, using vectors above, with `data.frame` function:
```{r, warning=FALSE, message=FALSE}
...
To view a specific column in a data frame we can use *$* such that:
```{r, warning=FALSE, message=FALSE}
df_1 = data.frame(vector1, vector2) #create data frame called df_1
df_1$vector1
```
We can see that columns in the *df_1* adapted names of the vectors. 
```{r, warning=FALSE, message=FALSE}
colnames(df_1)
```
