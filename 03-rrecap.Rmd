...
a <- c(1:10)
```
It is also possible to create a vector with text-based obejcts, for example with capitals from our `afrilearndata`:
```{r, warning=FALSE, message=FALSE}
...
```
### List
List can be though of as a container where various types of objects can be stored at the same type (unlike in vector). 
### Matrix
