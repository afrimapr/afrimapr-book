...
Previous section demonstrated what can be achieved with the learning materials provided in this book and gave an example of capabilities of packages that are used. In contrast, this section presents basic steps that are required (or simply a good practice) to take before starting the project where it provides a brief recap of R fundamentals such as data types and structures, objects and commands. It aims at building a starting point for those who are new to R and provide basic background knowledge that the reader can build on, while progressing through the further chapters of this book. It will be particularly useful to beginners, who have minimal knowledge of R. Therefore, more advanced readers can move forward to the next chapter where spatial data types are looked into. <br/>

This chapter proposes several **learning objectives**. In particular, at the end of this chapter, the reader will be able to:

- create a working directory for his/her project
- create, name and save a new R script
- execute R code
- install and load an R package
- import data set into R space
- identify and characterise different data types
- classify and contrast various data structures
- accessing and using `help` within R 
...
```
Further, we have to load it into R using `library()` function:
```{r, eval=FALSE}
library(remotes)
```
...
<div class = "blue">
Please note the presence of the quotation mark *" "* in case of the first function and its lack in case of the second function. 
</div>
...
For example, install packages containing data set:
```{r, eval=FALSE}
remotes::install_github("afrimapr/afrilearndata")
```
Load them:
...
Importantly, in order to get help regarding objects with unusual names such as the operators, it is necessary to use quotation marks:
```{r, warning=FALSE, message=FALSE}
?"&"
...
R documentation for functions frequently offers a working example of the function which can be accessed with:
```{r, warning=FALSE, message=FALSE}
example("&")
```
...
## Wrap up
In this chapter we learned several R commands which prepared the reader for starting with his own project. Moreover, we familiarised with a number of data type and R objects. Moreover, we looked at how to obtain help and access R documentation. The material covered in this section aimed at building a base for the reader to allow him?her successfully progress through the book. 
